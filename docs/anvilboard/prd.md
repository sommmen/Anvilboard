...
|---|---|---|---|---|
| 1 | Which authentication model best preserves self-hosted simplicity while supporting workspace-scoped people and automation credentials? | Architecture and security | Before foundation implementation | Resolved — local username/password credentials and workspace-scoped API tokens are the initial mechanisms; `ICredentialProvider` remains the seam for future OIDC/SSO. Passwords and tokens are stored only as salted hashes, and bootstrap is a one-time local flow. |
| 2 | What workflow configuration freedom is allowed while retaining cross-workspace reporting and provider mappings? | Product and domain | Before migration design | Resolved — workspace administrators may define named states, ordering, terminal/archive flags, and explicit directed transitions; state keys are stable, provider mappings use a separate provider-state mapping, and cross-workspace reports aggregate by normalized state category rather than display name. |
| 3 | For each provider, which fields are imported, locally editable, provider-controlled, or eligible for future write-back? | Product and integration | Before integration productization | Resolved — provider identity, URL, provider state, provider timestamps, and source metadata are provider-controlled/read-only; title, description, assignee, labels, comments, and workflow state are locally editable only when the integration write-back policy explicitly enables that field. Initial GitHub/Linear policy is import-only. |
| 4 | What backup target, retention period, encryption expectation, and restore RPO/RTO are required for pilot workspaces? | Operations and pilot users | Before pilot | Resolved — support verified export to a user-selected local file plus an operator-managed copy; backups contain a manifest, checksums, schema version, and redacted secrets (no plaintext secret values are ever written to a backup — see `ISecretStore`/Data Protection in tech-design), target RPO ≤ 24 hours and RTO ≤ 4 hours for pilot workspaces, and retain at least 7 daily and 4 weekly copies when scheduled backups are enabled. |
| 5 | What exact pilot cohort and observed task baseline will validate the value hypothesis in §6 and §15? | Product | Before pilot recruitment | Resolved — run a 4-week pilot with 5–10 workspaces, 20–50 active users, and at least 1,000 imported issues; record baseline triage time, stale-item age, and weekly active usage before onboarding, then require measurable improvement in two of the three target outcomes before expanding scope. |
| 6 | What plugin trust and distribution model is acceptable for third-party assemblies in self-hosted deployments? | Security and platform | Before third-party plugin support | Resolved — first-party plugins ship in the product; third-party plugins are opt-in, administrator-installed from a local signed package, require a manifest/contract-version/capability validation, run in-process only as trusted code, and are disabled on load failure. No remote marketplace or sandbox is in the pilot scope. |
