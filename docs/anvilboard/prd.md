...
| 4 | What backup target, retention period, encryption expectation, and restore RPO/RTO are required for pilot workspaces? | Operations and pilot users | Before pilot | Resolved — support verified export to a user-selected local file plus an operator-managed copy; backups contain a manifest, checksums, schema version, and redacted secrets, are encrypted by the configured secret provider, target RPO ≤ 24 hours and RTO ≤ 4 hours for pilot workspaces, and retain at least 7 daily and 4 weekly copies when scheduled backups are enabled. |
