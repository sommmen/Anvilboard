...
| Workflow Engine | Defines/validates configurable workflow states and transitions; migrates legacy `IssueStatus` values; exposes workspace-scoped REST and CLI/MCP administration with service-owned audit events | `IWorkflowService`, `/api/workflow/*`, `workflow` agent operations | Domain (`WorkflowState`), Infrastructure, Audit | [`docs/features/workflow-engine.md`](../features/workflow-engine.md) |
...
>
> **Last verified:** 2026-09-12 against the board-experience-parity change set — 521 populated .NET tests passing,
> 0 failing; `npm test` in `src/anvilboard-web` runs 44 passing and `npm run build` succeeds. Re-stamp this line
> whenever a milestone's implementation state changes (INFO-005).

| Milestone | Tasks | Estimate | Status |
|---|---|---|---|
| M1: Workspace & Auth Foundation | Workspace/role model, auth middleware, cross-workspace isolation tests | 3 weeks | Implemented — model, middleware, isolation tests, admin-triggered revocation, CLI/MCP enforcement parity, and workspace-bound filtering on REST/application queries (MAJ-022, see `workspace-authorization.md`) |
| M2: Configurable Workflow | `WorkflowState`/`WorkflowTransition` model, migration from `IssueStatus`, transition enforcement | 2 weeks | Implemented — workspace-scoped administration, audited configuration mutations, arbitrary state-ID transitions across REST/CLI/MCP, and configured-state selection in the issue detail UI are implemented and tested (see `workflow-engine.md`) |
...
| M3.5: Extended Ticket Model & List View | Free-form type/priority migration, session-state fields, threaded comments, Linear-style list view w/ grouping and ordering | 2 weeks | Partial — free-form type/priority migration is done, and the list view now offers the full filter/grouping/ordering set over `GET /api/board`; threaded comments are still flat (no reply-to) (see `issue-board-service.md`) |
...
| M4.5: Artifacts & Issue Linking | `Artifacts`/`IssueLinks` schema (`Type`+`Description`, `BLOCKS` dependency projection), `IArtifactStore` abstraction (SQLite-backed), artifact/link CRUD endpoints | 2 weeks | Partial — schema, `IArtifactStore`, and full issue-link CRUD (create/list/update/remove, plus a canonical link-type vocabulary) are implemented and tested; there is no artifact application service (attach/list/remove) (see `artifacts.md`, `issue-linking.md`) |
...
| M6.7: GitHub PR Artifacts & Plugin Persistence | GitHub plugin PR correlation/refresh as a `pull_request` artifact, `IPluginConfigStore`/`IPluginStateStore` abstractions | 1 week | Partial — `IPluginConfigStore`/`IPluginStateStore` and their SQLite-backed implementation are implemented and tested, and the `ArtifactService.RefreshArtifactAsync` dedup-key upsert seam that PR correlation consumes is now implemented (see [`../plans/artifact-service.md`](../plans/artifact-service.md) and `artifacts.md`); the GitHub plugin does not yet call it, so PR correlation/refresh itself remains outstanding |
| M7: Backup/Restore | Backup export, restore integrity verification, audited recovery drill | 1 week | **Implemented** — delivered per [`../plans/backup-and-restore.md`](../plans/backup-and-restore.md): `IBackupService` create/verify/restore, fail-closed restore with a `.pre-restore` safety copy, admission control and drain via `IRestoreCoordinator`, REST endpoints under `/api/backups`, and agent create/list/verify operations (no agent restore, by design). Recovery drill documented in [`../../DEVELOPMENT.md`](../../DEVELOPMENT.md) |
...
| OQ-002 | What is the exact idempotency-key retention window? | Resolved | Retain successful and failed terminal outcomes for 30 days, keyed by the composite tuple `(WorkspaceId, ActorId, Operation, Key)` (§10.3 Index Strategy), with a canonical request hash stored per key. Replays of the same tuple within the window return the stored outcome; a payload change under the same tuple returns `IDEMPOTENCY_KEY_REUSED`. A different actor produces a distinct key tuple entirely, so it is a new record, never a reuse rejection. Expired rows may be purged by a scheduled maintenance job and are never reused silently. | 2026-09-07 |
