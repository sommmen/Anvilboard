...
> 0 failing; `npm test` in `src/anvilboard-web` 21 passing. Re-stamp this line whenever a milestone's
> implementation state changes (INFO-005).

| Milestone | Tasks | Estimate | Status |
|---|---|---|---|
| M1: Workspace & Auth Foundation | Workspace/role model, auth middleware, cross-workspace isolation tests | 3 weeks | Partial — model, middleware, isolation tests, admin-triggered revocation, and CLI/MCP enforcement parity are all implemented; the remaining gap is workspace-bound filtering on REST/application queries (MAJ-022, see `workspace-authorization.md`) |
