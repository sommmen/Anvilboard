...
| 10 | real-time publisher, coalescer, workspace hub, client sync | application service / transport boundary | `Anvilboard.Application.Tests/Realtime/`; `Anvilboard.Api.Tests/Realtime/`; `anvilboard-web/src/app/core/realtime-board-sync.service.spec.ts` | No | None — the component is unimplemented; see [realtime-updates-implementation-plan.md](../features/realtime-updates-implementation-plan.md) |
...
| Testable boundaries with an existing test file | 8 of 10 listed above have at least one test file |
...
Derived from [realtime-updates.md](../features/realtime-updates.md) `AC-RT-001`–`AC-RT-006`; phase
and file targets are in [realtime-updates-implementation-plan.md](../features/realtime-updates-implementation-plan.md).

| TC ID | Module | Title | Dimensions | Expected result | Priority | Infra | Automation |
|---|---|---|---|---|---:|---|---|
| TC-RT-001 | Realtime publication | Committed mutation publishes exactly one workspace-scoped versioned change | commit vs. rollback | A committed issue mutation yields one `RealtimeIssueChange` carrying workspace, issue identity, version, and correlation id; a mutation that throws before commit yields none. | P1 | recording fake publisher + SQLite | Planned |
| TC-RT-002 | Hub authorization | Workspace isolation holds at connection and delivery | foreign workspace, unauthenticated | An unauthenticated connection is rejected with `WORKSPACE_ACCESS_DENIED`; a workspace-A client receives no workspace-B event and has no client-callable method to join an arbitrary group. | P1 | `WebApplicationFactory` + hub client | Planned |
| TC-RT-003 | Mutation isolation | Slow or disconnected client neither delays the mutation nor grows unbounded work | deliberately slow client | Mutation-response latency is unchanged versus a no-client baseline; queued work stays bounded and drop/coalesce counters increment instead. | P1 | `WebApplicationFactory` + throttled hub client | Planned |
| TC-RT-004 | Coalescing | Burst of same-issue updates is coalesced without a full board redraw | burst load | N rapid changes to one issue deliver a bounded number of notifications carrying the highest version; the client patches the affected entry in place rather than re-rendering the whole list. | P1 | coalescer unit fixture + Angular component test | Planned |
| TC-RT-005 | Reconnect recovery | Reconnecting client converges by documented re-fetch, not server replay | connection drop | Reconnect triggers exactly one board re-fetch; the server performs no event replay and retains no per-client backlog. | P1 | Angular component test + hub client | Planned |
...
| FR-INT-006 | **Gap:** the plugin-event relay (`AC-RT-006`) is deferred to the optional Phase 7 of the real-time implementation plan; `IPluginEventPublisher` does not exist yet. | Planned gap |
...
| [realtime-updates.md](../features/realtime-updates.md) | AC-RT-006 | **Gap:** plugin-event relay deferred with FR-INT-006. | Planned gap |
...
| Plugin-event relay coverage (`FR-INT-006`, `AC-RT-006`) | `IPluginEventPublisher` does not exist in `Anvilboard.Plugins.Abstractions`, so the relay path cannot be exercised. | Add the relay test case with Phase 7 of [realtime-updates-implementation-plan.md](../features/realtime-updates-implementation-plan.md). |
