...
> coverage: `dotnet test Anvilboard.slnx` reports **320 passing, 0 failing**. The tables below have
...
| **Input Mode** | Code Mode — verified against the actual `dotnet test` run for `Anvilboard.slnx` (320 passing, 0 failing) |
...
| Total automated tests (last run) | 341 total: 320 .NET (`dotnet test Anvilboard.slnx` — Application 192, Infrastructure 41, Agent 40, Api 30, GitHub 12, Linear 5) plus 21 Angular (`ng test`, 3 spec files), all passing |
...
| TC-RT-005b | Reconnect backoff | Manual reconnect retries back off exponentially and reset after success | repeated failures vs. a recovered connection | Each consecutive manual reconnect failure doubles the retry delay from 1s up to a 30s cap; a successful (re)connect resets the delay back to 1s. | P2 | Angular service test with fake timers | Automated (`realtime-board-sync.service.spec.ts` "backs off exponentially across consecutive reconnect failures, capped at 30s", "resets the backoff delay to 1s after a successful reconnect following failures") |
| TC-RT-005c | Stale-connection reconnect guard | A close event from a superseded connection cannot trigger a duplicate reconnect cycle | close event fired on a connection that is no longer the active one | The `onclose` handler only schedules a reconnect when the closing connection is still the service's current connection; a close event from an already-replaced connection is ignored. | P2 | Angular service test with multiple fake connections | Automated (`realtime-board-sync.service.spec.ts` "ignores a close event from a connection that has already been replaced") |
...
| TC-RT-006b | Trusted/public publisher isolation | A reflection-loaded plugin cannot resolve the trusted, workspace-authenticated publisher, nor any raw/collection escape hatch, and the public contract is a safe no-op | host-only marked capability, raw `IServiceProvider`/`IServiceScopeFactory`, `IEnumerable<T>` collection resolution, and an ordinary constructor dependency | A plugin constructor requesting a service marked `IHostOnlyPluginCapability` (e.g. `ITrustedPluginEventPublisher`), the raw `IServiceProvider`/`IServiceScopeFactory`, or an `IEnumerable<T>` of a host-only capability all fail to construct rather than receiving the host's instance or an unfiltered container; an ordinary constructor dependency still resolves normally; `IPluginEventPublisher` resolves to `PublicPluginEventPublisher`, an intentional no-op distinct from the trusted publisher, since a reflection-loaded plugin has no host-authenticated workspace context of its own. | P1 | `PluginRegistry` unit fixture + realtime DI fixture | Automated (`PluginRegistryTests.ReflectionLoadedPlugin_CannotResolveHostOnlyCapability`, `ReflectionLoadedPlugin_CanStillResolveOrdinaryServices`, `ReflectionLoadedPlugin_CannotResolveRawServiceProvider`, `ReflectionLoadedPlugin_CannotResolveHostOnlyCapabilityCollection`; `PluginEventRelayTests.AddRealtime_PublicPluginPublisherIsDistinctFromTrustedPublisher`, `AddRealtime_PublicPluginPublisher_DropsCallerSelectedWorkspaceEvent`) |
