...
| TC-RT-006b | Trusted/public publisher isolation | A reflection-loaded plugin cannot resolve the trusted, workspace-authenticated publisher, but the public contract still delivers | host-only marked capability vs. an ordinary constructor dependency | A plugin constructor requesting a service marked `IHostOnlyPluginCapability` (e.g. `ITrustedPluginEventPublisher`) fails to construct rather than receiving the host's instance; an ordinary constructor dependency still resolves normally; `IPluginEventPublisher` resolves to `PublicPluginEventPublisher`, distinct from the trusted publisher, and still relays approved events through the same `PluginEventRelay`. | P1 | `PluginRegistry` unit fixture + realtime DI fixture | Automated (`PluginRegistryTests.ReflectionLoadedPlugin_CannotResolveHostOnlyCapability`, `ReflectionLoadedPlugin_CanStillResolveOrdinaryServices`; `PluginEventRelayTests.AddRealtime_PublicPluginPublisherIsDistinctFromTrustedPublisherButStillRelays`, `AddRealtime_PublicPluginPublisher_RelaysApprovedEventsLikeTheTrustedPublisher`) |
