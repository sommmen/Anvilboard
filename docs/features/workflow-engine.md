...
- **Migration is rollback-safe**: both `Issues.Status` (deprecated) and `Issues.WorkflowStateId` remain populated and readable for one complete release; `Status` is dropped only after migration verification, a successful backup/restore drill, and a repository-wide consumer check confirm no remaining reads or writes (§10.4 step 4/5).
