...
- `IPluginEventPublisher` (the public contract) resolves to `PublicPluginEventPublisher`, which is an intentional no-op. A reflection-loaded plugin has no host-authenticated workspace context of its own, so accepting the `WorkspaceId` it supplies on a `PluginEvent` would let it notify a workspace other than its own; the public contract stays source-compatible for a plugin to call, but nothing it publishes ever reaches a client. When realtime is not enabled, `IPluginEventPublisher` falls back to `NullPluginEventPublisher`, which behaves identically.
...
| AC-IPP-116 | P1 | Given a plugin writing to `IPluginConfigStore`/`IPluginStateStore` under its own `(workspaceId, pluginKey)`, when another plugin attempts to read that same key. | The read returns nothing/is denied; only the owning plugin can read or write its namespaced config/state. | Unit — `PluginConfigStoreTests.CrossPluginRead_IsDenied` (negative, FR-INT-007). |
