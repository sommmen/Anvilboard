...
| `Pre*` hook denies the gated operation, or exceeds its budget (treated as `Deny`) | `HOOK_VETO_DENIED` | 409 | Names the denying hook and its stated reason; the gated operation (e.g. phase change, attachment add) does not proceed. |
