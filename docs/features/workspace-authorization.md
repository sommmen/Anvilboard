...
- **Access-denied status decision**: authenticated actors outside a workspace or lacking permission always receive `403 WORKSPACE_ACCESS_DENIED`; `404 REFERENCED_ENTITY_NOT_FOUND` is reserved for missing resources inside an authorized workspace. Concretely (MAJ-022): an ID-addressed route answers `403` for a *caller-supplied* id that is either foreign **or** nonexistent, because replying `404` to only one of those cases would let a caller enumerate which ids exist in other workspaces. `404` therefore applies only to a *dependent* lookup that fails after scoping has already succeeded — a missing workflow state on a status change, or a link id not attached to an already-scoped issue.
...
| AC-301 | P0 | Given an unfiltered `GET /api/issues`, when the caller is authenticated against workspace A | Only workspace A's issues are returned; no key, title, or description from another workspace appears | Integration: `CrossWorkspaceIsolationEndpointTests.ListIssues_NeverIncludesAnotherWorkspacesIssues` |
| AC-302 | P0 | Given an unfiltered `GET /api/dashboard/summary` | Every count and grouping is computed from workspace A's issues alone | Integration: `DashboardSummary_CountsOnlyTheCallersOwnWorkspace` |
| AC-303 | P0 | Given a list or summary request whose `teamId`/`assigneeId` filter names another workspace's entity | `403 WORKSPACE_ACCESS_DENIED`; an empty `200` is not acceptable because it still confirms the id exists | Integration: `ListIssues_FilteredByAnotherWorkspacesTeam_IsDenied`, `DashboardSummary_ScopedToAnotherWorkspacesTeam_IsDenied` |
| AC-304 | P0 | Given `GET /api/issues/{id}` for an issue in another workspace | `403 WORKSPACE_ACCESS_DENIED`; the body carries no issue fields | Integration: `GetIssue_FromAnotherWorkspace_IsDeniedIndistinguishablyFromAnUnknownId` |
| AC-305 | P0 | Given a mutation (`status`, `assign`, `comment`) addressed to another workspace's issue | `403` **and** the target row is byte-for-byte unchanged; no comment or activity row is written | Integration: `ChangeStatus_…`, `Assign_…`, `AddComment_OnAnotherWorkspacesIssue_IsDeniedAndWritesNoComment` |
| AC-306 | P0 | Given `POST /api/issues` naming another workspace's `teamId` | `403`; no issue row is created and the foreign team's `NextIssueNumber` is unchanged | Integration: `CreateIssue_IntoAnotherWorkspacesTeam_IsDeniedAndCreatesNothing` |
| AC-307 | P0 | Given any artifact or link route addressed to another workspace's issue | `403`; no `Artifacts`/`IssueLinks` row is added or removed | Integration: `ListArtifacts_…`, `AttachArtifact_…`, `ListLinks_…`, `CreateLink_BetweenTwoOfAnotherWorkspacesIssues_IsDeniedAndWritesNoLink` |
| AC-308 | P0 | Given an ID-addressed route called with a foreign id and again with a nonexistent id | Both produce an identical status **and** an identical response body, so the route is not an existence oracle | Integration: `GetIssue_FromAnotherWorkspace_IsDeniedIndistinguishablyFromAnUnknownId` (compares bodies string-for-string) |
| AC-310 | P0 | Given polling ingestion, which has no authenticated workspace | `UpsertFromExternalUnscopedAsync` still resolves a team by source key and fails closed on zero or multiple matches; `SyncCoordinator` tests pass unmodified | Regression: `Anvilboard.Application.Tests` sync/workflow suites |
